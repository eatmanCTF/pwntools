...
class Pwn(Logger):
	...
	def __init__(self, elf, mode, **kwargs):
		...
		if mode == "src":	
			...
			self._libraires = [f"{LIBC_DBSRCROOT}/{self.elf.arch}/{self._src}/lib/{lib.strip()}" for lib in libraires]
		else:
			...
			if not self._ld_path:
				if self._libc_path:
					...
					if libc_id:
						ld_id = f"ld_{libc_id}"
						self._ld_path = os.path.join(LIBC_DBROOT, f"{ld_id}.so")
						if os.path.isfile(self._ld_path):
							self.warn("{} was found to load {}".format(self._ld_path, libc_id))
	...
	def local(self, argv, *a, **kw):
		...
		if args.GDB:
			return gdb.debug([self.elf.path] + argv, gdbscript=self._gdbscript, env=env, *a, **kw)
		else:
	...
	def get_libc_version(libc):
		...
		p = subprocess.Popen("{}/identify '{}' | grep -v 'ld_'".format(LIBC_DATABASE_UTIL, libc_abspath), 
	...
	def change_ld(binary, ld):
		...
		if not os.path.isfile(ld):
			if not ld in ["2.23", "2.24", "2.25", "2.26", "2.27", "2.28", "2.29", "2.30", "2.31"]:
...
class MYFFI(CDLL):
	...
	def mktime(self, timestruct):
		...
		p = subprocess.Popen(f"{MYFFI.mktimeelf} {' '.join([str(i) for i in timestruct])}", 
			shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		(stdout, stderr) = p.communicate()
		if stderr:
